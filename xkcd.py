...
def get_pw_strength(password):
    ...
    log.info(password + ": entropy bits = " + str(round(stats.entropy_bits)) + "; entropy density = " + str(
        stats.entropy_density) + "; strength = " + str(stats.strength()))
...
def time_to_exhaust_search_space(possible_combinations, passwords_per_sec_billions=350):
    ...
    if seconds_to_exhaust < 1.0:
        log.info("Less than one second required to exhaust search space at a rate of " + str(
            passwords_per_sec_billions) + " billion passwords per second.")
    else:
        log.info("Time to exhaust search space = " + display_time(
            seconds_to_exhaust, granularity=1) + " at " + str(
            passwords_per_sec_billions) + " billion passwords per second.")
...
def get_xkcd_entropy(words, num_words_in_password):
    ...
    log.info("Number of possible combinations of words = " + "{:,}".format(possible_combinations) + " (" + str(
        len(str(possible_combinations))) + " decimal digits) or " + bin(possible_combinations) + " (" + str(
        len(str(bin(possible_combinations))) - 2) + " binary digits)")
...
def main(args=sys.argv[1:]):
    ...
    for n in range(0, args.ctpw):
        ...
        pw = create_xkcd_password(filename=args.wordlist, num_words_in_password=int(args.numwords),
                                  ...
                                  notcontain=args.notcontain, args=args)
        pw = pw.replace(" ", "")
        passwords.append(pw)
